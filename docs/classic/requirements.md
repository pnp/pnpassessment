...
`--homepageonly` | Only assess the home page of each web, rather than every classic page. Physical ASPX discovery still runs for the selected scope; non-home pages are retained in `discovery.csv` with `AssessmentStatus=NotSelected`. Useful for a fast home-page modernization-readiness assessment.
